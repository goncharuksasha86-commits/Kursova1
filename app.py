...
if uploaded:
    ...
    if col:
        ...
        # 1. Шукаємо стовпець часу (враховуючи переклади та регістр)
        for c in df.columns:
        ...
        # 2. Якщо за назвою не знайшли, беремо найпершу колонку таблиці (де зазвичай і є час)
        ...
        if time_col:
            ...
            # РІШЕННЯ: Групуємо строго за часовою колонкою, незалежно від selectbox!
            # Рахуємо кількість подій на кожну секунду/хвилину
            chart_data = df.groupby(time_col).size().to_frame(name="Граф 1")
            
            # Будуємо фірмовий інтерактивний графік Streamlit
            st.bar_chart(chart_data, color="#0066cc")
        else:
            st.warning("⚠️ Не вдалося визначити стовпець часу в таблиці для побудови діаграми активності.")
        # ----------------------------------------------------
